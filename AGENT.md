...
a minimal netinstaller (`launcher/` + `build/launcher/`), kernel tuning
(`kernel/`), six Bash+YAD apps (`apps/`), a custom `.var` data format
(`lib/`, `tools/`, `tests/`), artwork (`assets/`), guides (`docs/`), and CI
that lints + builds the ISOs on every push.
...
build/build-launcher.sh   minimal launcher ISO build (console netinstaller)
build/launcher/           launcher profile (lb config, package list, tty1 hook)
scripts/                  installer modules; also shipped inside the ISO
launcher/arun-launcher    netinstaller TUI (whiptail or text, DRY_RUN=1 for plans)
launcher/install-engine.sh debootstrap+partition+grub engine (sourced, not run)
launcher/*.var            version registry + accounts database (.var backend)
lib/var.sh                .var parser library (source it; bash 4+)
tools/var                 .var CLI: get/has/keys/dump/set/del/validate
tests/test-var.sh         .var test suite (54 assertions; CI runs it)
...
find build scripts apps lib tools tests launcher -type f \
  \( \( -name '*.sh' -o -name '*.hook.chroot' -o -name 'arun-*' \
       -o -name 'var' \) ! -name '*.desktop' \) \
  -exec bash -n {} + \
  && bash -n build/build-iso.sh && bash -n build/lb-config.sh \
  && bash -n build/build-launcher.sh \
  && bash -n build/launcher/lb-config-launcher.sh

# 2. fast linter (syntax + hooks + lb flags + packages + tests + .var)
bash scripts/lint-distro.sh

# 3. .var suite on its own (also part of the linter)
bash tests/test-var.sh

# 4. no emojis anywhere in tracked text files (repo rule - see below)
python3 - <<'EOF'
...
# 5. icon still valid XML
python3 -c "import xml.dom.minidom; xml.dom.minidom.parse('assets/arunlinux.svg')"

# 6. launcher engine dry-run (prints the install plan, touches nothing)
bash -c '. lib/var.sh; . launcher/install-engine.sh; DRY_RUN=1 eng_partition_guided sdz && DRY_RUN=1 eng_format'
```

## CI behavior (important)

- Lint + full ISO build trigger on **every push to every branch** plus manual
  dispatch. Full ISO also runs weekly (Sunday 02:00 UTC).
...
2. **Public arch name is `x86_64`, Debian-internal name stays `amd64`.**
   ISO files: `arunlinux[-launcher]-x86_64-v<commit>.hybrid.iso`. Never rename
   the Debian arch in lb flags, package lists, or repo lines or builds break.
3. **Live user is `arun`, hostname `arunlinux`** (lb-config `--bootappend-live`
   + preseed must agree). The launcher ISO boots to a root console instead.
...
10. **`.var` data rules:** `key = value;` per VAR-SPEC v1, validated by
    `tools/var validate` in CI. Never store passwords in `.var` files.
    `set`/`del` only touch the top file, never includes.
11. **Installer engine safety:** destructive steps go through `_run`
...
- To disable the Debian installer in a profile, OMIT `--debian-installer`
  entirely (default = none) — the launcher profile does this.
...
- Host that builds any ISO must BE Debian 13 (live-build rule); CI uses a
...
- **New package:** append to the right `build/package-lists/*.list.chroot`
  (or `build/launcher/*.list.chroot` for the installer image), one name per
  line, `#` comments allowed. The linter verifies it against real trixie
  repos. Prefer small native .debs; keep the full ISO < 4 GB.
...
- **New app:** `apps/<name>/<name>` (executable Bash) + optional `.desktop`
  ...
  and add it to the `.desktop` link loop in both files.
- **New registry version:** append a slot to `launcher/versions-registry.var`,
  bump `ver.count`, set `registry.latest`, run `tools/var validate` on it.
- **New engine step:** add `eng_*` function in `launcher/install-engine.sh`,
  route destructive commands through `_run`, call it from `flow_install` in
  `launcher/arun-launcher`, extend the DRY_RUN smoke test mentally (no
  destructive call outside `_run`).
